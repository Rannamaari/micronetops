            ...
            <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg p-4 sm:p-6">
                <form method="POST" action="{{ route('leads.update', $lead) }}" class="space-y-4">
                    ...
                    <div>
                        ...
                        <input id="phone" name="phone" type="text" value="{{ old('phone', $lead->phone) }}" required
                               class="block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 shadow-sm text-sm
                                      focus:border-indigo-500 focus:ring-indigo-500">
                        @error('phone')
                            <p class="mt-1 text-xs text-red-600 dark:text-red-400">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <textarea id="address" name="address" rows="2"
                                  class="block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 shadow-sm text-sm
                                         focus:border-indigo-500 focus:ring-indigo-500">{{ old('address', $lead->address) }}</textarea>
                        @error('address')
                            <p class="mt-1 text-xs text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <select id="interested_in" name="interested_in" required
                                    class="block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 shadow-sm text-sm
                                           focus:border-indigo-500 focus:ring-indigo-500">
                                <option value="moto" {{ old('interested_in', $lead->interested_in) === 'moto' ? 'selected' : '' }}>Motorcycle Service</option>
                                <option value="ac" {{ old('interested_in', $lead->interested_in) === 'ac' ? 'selected' : '' }}>AC Service</option>
                                <option value="both" {{ old('interested_in', $lead->interested_in) === 'both' ? 'selected' : '' }}>Both</option>
                            </select>
                            @error('interested_in')
                                <p class="mt-1 text-xs text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
                    ...
                    <div>
                        ...
                        <textarea id="notes" name="notes" rows="3"
                                  class="block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 shadow-sm text-sm
                                         focus:border-indigo-500 focus:ring-indigo-500"
                                  placeholder="Vehicle details, requirements, conversation notes...">{{ old('notes', $lead->notes) }}</textarea>
                        @error('notes')
                            <p class="mt-1 text-xs text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
