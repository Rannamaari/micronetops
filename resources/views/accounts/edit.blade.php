                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form method="POST" action="{{ route('accounts.update', $account) }}" class="space-y-4">
                        ...
                        <div>
                            ...
                            <input name="name" class="mt-1 w-full rounded border-gray-300" value="{{ $account->name }}" required>
                        </div>
                        <div>
                            <label class="block text-sm font-medium">Type</label>
                            <select name="type" class="mt-1 w-full rounded border-gray-300" required>
                                @foreach ($types as $key => $label)
                                    <option value="{{ $key }}" @selected($account->type === $key)>{{ $label }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div>
                            <label class="block text-sm font-medium">Notes</label>
                            <textarea name="notes" rows="2" class="mt-1 w-full rounded border-gray-300">{{ $account->notes }}</textarea>
                        </div>
                        ...
                        <div class="flex items-center justify-end gap-3">
                            <a href="{{ route('accounts.index') }}" class="text-gray-600 hover:underline">Cancel</a>
