                ...
                <form method="POST" action="{{ route('customers.update', $customer) }}" class="space-y-4">
                    ...
                    <div>
                        ...
                        <textarea id="address" name="address" rows="3"
                                  class="block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 shadow-sm text-sm
                                         focus:border-indigo-500 focus:ring-indigo-500"
                                  placeholder="Example: H. Paradise, Boduthakurufaanu Magu, Malé">{{ old('address', $customer->address) }}</textarea>
                        @error('address')
                    ...
                    <div>
                        ...
                        <select id="category" name="category"
                                class="block w-full rounded-md border-gray-300 shadow-sm text-sm
                                       focus:border-indigo-500 focus:ring-indigo-500">
                            <option value="moto"  {{ old('category', $customer->category) === 'moto' ? 'selected' : '' }}>Motorcycle</option>
                            <option value="ac"    {{ old('category', $customer->category) === 'ac' ? 'selected' : '' }}>AC Service</option>
                            <option value="both"  {{ old('category', $customer->category) === 'both' ? 'selected' : '' }}>Both</option>
                        </select>
                        @error('category')
                            <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <textarea id="notes" name="notes" rows="3"
                                  class="block w-full rounded-md border-gray-300 shadow-sm text-sm
                                         focus:border-indigo-500 focus:ring-indigo-500">{{ old('notes', $customer->notes) }}</textarea>
                        @error('notes')
                            <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
